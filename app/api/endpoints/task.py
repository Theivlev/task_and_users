...
async def read_task(task_id: int,
    ...
    if task is None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Запись не найдена")
    return task
...
async def update_task(task_id: int, modified_task: TaskUpdate,
    ...
    if task is None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Запись не найдена или вы пытаетесь редактировать чужую запись!")
    return task
...
async def delete_task(task_id: int,
    ...
    if task is None:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Запись не найдена или вы пытаетесь удалить чужую запись!")
    return task